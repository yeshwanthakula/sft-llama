...
def fsdp_setup(use_fsdp=False):
    ...
    if not use_fsdp:
        return 0, 0, 1, True, "cpu"  # Default values for non-DDP training
    
    # Check for CUDA availability for DDP
    assert torch.cuda.is_available(), "DDP requires CUDA"
    
    # Initialize process group , change the naming
...
def train_llama(
  cfg=LLAMA32_CONFIG_1B,
    ...
    micro_batch_size=64,         # Higher for cloud hardware
    seq_length=1024,               # Optimal for Python functions
    ...
    generate_interval=5,        # Every 5 steps
    use_compile=True,             # Enable on cloud
    ...
    warmup_steps = max(1, int(0.1 * max_steps))  # 10% of steps for warmup
    ...
    if use_fsdp:
        ...
        model = FSDP(model, auto_wrap_policy=auto_wrap_policy, sharding_strategy=FSDP.ShardingStrategy.FULL_SHARD, device_id=ddp_local_rank)
        dist.barrier()
    ...
    try:
        ...
        for step in range(max_steps):
            ...
            if step % validation_interval == 0 or last_step:
                ...
                if master_process:
                    print(f"Step {step}: validation loss: {val_loss_item:.4f}")
